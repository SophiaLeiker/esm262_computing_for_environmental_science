...
* **ADDING CAR HERE**: additional super light car with mass 5,000 kg and a surface area 10m2
- We will call this car, car D
...
source(here('Assignments', "Assignment2", "power_gen_new.R"))
...
possible_cars$power = power_gen_new(V=speed_base, A = possible_cars$area, m=possible_cars$mass)
...
# assume a log normal distribution of speeds with mean 80km/hr{100km/hr}, and standard deviation that is 8% of the mean (b/c new SD is 10km/hr)
...
nsample = 200
mean_speed = log(80*0.277) #changed this to 80km/hr

speeds = rlnorm(mean=mean_speed, sd=0.8*mean_speed, nsample)
summary(speeds)
...
powerA = power_gen_new(V=speeds, A = possible_cars$area[1], m=possible_cars$mass[1])
# lets look at what we get
summary(powerA)

# next car (row 2)
powerB = power_gen_new(V=speeds, A = possible_cars$area[2], m=possible_cars$mass[2])
# lets look at what we get
summary(powerB)

# next car (row 3)
powerC = power_gen_new(V=speeds, A = possible_cars$area[3], m=possible_cars$mass[3])
# lets look at what we get
summary(powerC)

# next car (row 4)
powerD = power_gen_new(V=speeds, A = possible_cars$area[4], m=possible_cars$mass[4])
# lets look at what we get
summary(powerD)
...
#SOPHIA NOTES: for every single car, you want it to run a power function for all the speeds, you want to repeat the auto function for each row for all the possible car, but also have additional inputs you want to run for each
#the second part is saying for each run, you want to run (on the right side it is doing this for the autopower function)
#so when you are running the pmap, for Area A in my function I want to use area becuase in the function we used A, then here we also need to delinate A. For A and m, you use all the possible areas and masses, but for speeds you use all the possible speeds that wer created above. 
powerall2 = pmap(list(A = possible_cars$area, m=possible_cars$mass), power_gen_new, V=speeds)
...
powerall2b = mapply(FUN=power_gen_new, A = possible_cars$area, m=possible_cars$mass, MoreArgs = list(V=speeds)  )
# we can add column names
...
method2_plot = ggplot(powerallr2, aes(car,power, fill=car))+geom_boxplot()+ggtitle("Method 1: Pmap - Range of Power Consumption for Each Car Type")
method2_plot
...
for (i in 1:nrow(possible_cars)) {
  powerall3[,i] = power_gen_new(A=possible_cars$area[i], m=possible_cars$mass[i], V=speeds)
}
...
# quick visualization
method3_plot = ggplot(powerallr3, aes(car,power, fill=car))+geom_boxplot()+ggtitle("Method 2: For Loop - Range of Power Consumption for Each Car Type")
...
ggarrange(method2_plot, method3_plot, nrow=2)
